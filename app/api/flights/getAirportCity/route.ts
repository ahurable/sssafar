  ...
  try {
    ...
    if (query) {
      suggestions = airportsCache.filter(airport =>
        ...
        airport.showName?.includes(query)
      )
    } else {
...
async function loadAirportsFromXLSX(): Promise<Airport[]> {
  try {
    ...
    const fileUrl = `${baseUrl}/data/Airport.xlsx` // Using City.xlsx
    ...
    const showNameColumn = columnNames.find(col =>
      col.toLowerCase().includes('showName') || col.toLowerCase().includes('show name')
    )
    ...
    const airports: Airport[] = data.map((row: any, index: number) => {
      ...
      const showName = showNameColumn ? row[showNameColumn] : row[columnNames[4]]
      return {
        iata: (iata || '').toString().trim(),
        name: (name || '').toString().trim(),
        city: (city || '').toString().trim(),
        country: (country || '').toString().trim(),
        showName: (showName || '').toString().trim()
      }
