...
// Function to ensure we have a default city
async function ensureDefaultCity() {
  try {
    // Try to find any existing city
    let defaultCity = await prisma.tourCity.findFirst();
    
    if (!defaultCity) {
      // Create a default city if none exists
      defaultCity = await prisma.tourCity.create({
        data: {
          name: "شهر پیش فرض",
          description: "شهر پیش فرض برای تورها"
        }
      });
      console.log("Created default city:", defaultCity.id);
    }
    
    return defaultCity.id;
  } catch (error) {
    console.error("Error ensuring default city:", error);
    throw new Error("Could not create default city");
  }
}
...
export const POST = async (request: NextRequest) => {
  try {
    ...
    // Ensure we have a default city for the required relation
    const defaultCityId = await ensureDefaultCity();
    ...
    const tourData: any = {
      ...
      // Required city relation - always use the default city
      city: {
        connect: { id: defaultCityId }
      },
    ...
    // Add tourCity relation if tourCityId is provided
    if (validatedData.tourCityId) {
      // Verify the tourCity exists before connecting
      const tourCityExists = await prisma.tourCity.findUnique({
        where: { id: validatedData.tourCityId }
      });

      if (tourCityExists) {
        tourData.tourCity = {
          connect: { id: validatedData.tourCityId }
        };
      } else {
        console.warn("TourCity not found:", validatedData.tourCityId);
      }
    }
