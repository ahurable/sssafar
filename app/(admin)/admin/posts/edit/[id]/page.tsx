    ...
    const fetchPost = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Set tables if they exist
          if (post.tables && post.tables.length > 0) {
            setTables(post.tables.map(table => ({
              id: table.id,
              title: table.title,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(result.error || "خطا در بروزرسانی پست")
      }
    } catch (error) {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="text-right">
            ...
            <p className="text-muted-foreground">پست خود را ویرایش کنید</p>
          </div>
        </div>
        <Button type="submit" form="post-form" disabled={loading}>
          {loading ? "در حال بروزرسانی..." : "بروزرسانی پست"}
        </Button>
      </div>
