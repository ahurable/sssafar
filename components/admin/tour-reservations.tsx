...
import { Search, Filter, Phone, User, Calendar, MoreVertical, Users } from "lucide-react"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
interface TourReservationsProps {
  tourId: string
  reservations: Reservation[]
}

export function TourReservations({ tourId, reservations }: TourReservationsProps) {
...
//   }
  ...
  const getTotalTravelers = (travelers: any) => {
  ...
  const handleStatusChange = async (reservationId: string, newStatus: string) => {
    try {
      const response = await fetch(`/api/tours/reservations/${reservationId}`, {
        method: "PATCH",
        headers: {
  ...
  const exportReservations = () => {
    // Simple CSV export
    const headers = ["نام", "نام خانوادگی", "شماره تماس", "وضعیت", "تاریخ ثبت", "تعداد مسافران"]
    const csvData = filteredReservations.map(res => [
      ...
      res.status,
      res.createdAt,
      getTotalTravelers(res.travelers).toString()
    ])
    ...
    a.href = url
    a.download = `tour-reservations-${tourId}.csv`
    a.click()
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="py-6">
        <CardContent>
          <h3 className="font-semibold text-lg mb-4">آمار رزروهای تور</h3>
      ...
      <Card className="py-6">
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            <span>درخواست‌های رزرو تور</span>
              ...
              filteredReservations.map((reservation) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
                    ...
                    <span>{reservation.createdAt.toString()}</span>
                  </div>
                  ...
                  {reservation.travelers && (
                    <div className="flex items-center gap-2 text-sm text-gray-600 mb-3">
                      ...
                      <span>
                        {getTotalTravelers(reservation.travelers)} مسافر
                        {reservation.travelers.adults && ` (${reservation.travelers.adults} بزرگسال`}
                        {reservation.travelers.children && `, ${reservation.travelers.children} کودک`}
                        {reservation.travelers.infants && `, ${reservation.travelers.infants} نوزاد)`}
                      </span>
                    </div>
                  )}
