// components/admin/edit-visa-form.tsx
"use client"
...
import { Plus, Trash2, ArrowRight, Upload, X, Image as ImageIcon, Save } from "lucide-react"
...
interface EditVisaFormProps {
  visa: VisaService
}

export function EditVisaForm({ visa }: EditVisaFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    features: visa.features.length > 0 ? visa.features : [""],
    requirements: visa.requirements.length > 0 ? visa.requirements : [""],
    documents: visa.documents.length > 0 ? visa.documents : [""],
    priority: visa.priority.toString(),
    priceTables: visa.priceTables ? JSON.parse(JSON.stringify(visa.priceTables)) : [] as PriceTable[],
    published: visa.published,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success("خدمت ویزا با موفقیت به‌روزرسانی شد")
        router.push("/admin/visa")
        router.refresh()
      } else {
        toast.error(result.error || "خطا در به‌روزرسانی خدمت ویزا")
      }
    } catch (error) {
      console.error("Error updating visa service:", error)
      toast.error("خطا در به‌روزرسانی خدمت ویزا")
    } finally {
      setLoading(false)
    }
  }
