// components/admin/visa-reservations.tsx
"use client"
...
import { Search, Phone, User, Calendar, MoreVertical, FileText } from "lucide-react"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
interface VisaReservationsProps {
  visaId: string
  reservations: Reservation[]
}

export function VisaReservations({ visaId, reservations }: VisaReservationsProps) {
  ...
  const handleStatusChange = async (reservationId: string, newStatus: string) => {
    try {
      const response = await fetch(`/api/visa/reservations/${reservationId}`, {
        method: "PATCH",
        headers: {
  ...
  const exportReservations = () => {
    const headers = ["نام", "نام خانوادگی", "شماره تماس", "وضعیت", "تاریخ ثبت", "یادداشت"]
    ...
    a.href = url
    a.download = `visa-reservations-${visaId}.csv`
    a.click()
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="py-6">
        <CardContent>
          <h3 className="font-semibold text-lg mb-4">آمار درخواست‌های ویزا</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600">
                {reservations.filter(r => r.status === "UNDER_REVIEW").length}
              </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem
                          onClick={() => handleStatusChange(reservation.id, "UNDER_REVIEW")}
                        >
                          در حال بررسی
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() => handleStatusChange(reservation.id, "DOCUMENTS_REQUIRED")}
                        >
                          نیاز به مدارک
                        </DropdownMenuItem>
                        <DropdownMenuItem
